...
import { useGetProject, useGetTechLang } from "@/lib/hooks";
...
export default function LandingPage({ openerSection }) {
    ...
    const { data: dataTechLang } = useGetTechLang();
    ...
    const techLangDataState = useMemo(() => {
        const dataMap = [dataTechLang?.web, dataTechLang?.api, dataTechLang?.mobile, dataTechLang?.ml];

        const selected = dataMap[skillActive];

        return {
            lang: selected?.lang?.join(" | ") ?? "",
        ...
        <div ref={sectionView}>
            <section className="lg:p-20 sm:p-10 xs:p-5 xs:pt-8 sm:pt-10 flex lg:flex-row xs:flex-col-reverse lg:justify-around lg:items-center lg:h-[90vh]">
                <div className="lg:w-1/2 xs:w-full">
                    ...
                    >
                        <span className="font-bold text-textMain lg:text-2xl xs:text-xl">I'm Sinamo Kevin Nathanael</span> a Software Engineering graduate with a strong focus on{" "}
                        <span className="font-bold">frontend web development</span>. While I specialize in web interfaces, I also build simple{" "}
                        <span className="font-bold">mobile and desktop apps</span>. Currently exploring the exciting world of <span className="font-bold">AI and machine learning</span>.
                    </motion.p>
                </div>
            ...
            <section className="lg:h-[80vh] flex flex-col lg:justify-center">
                <motion.h2 initial={{ x: -500 }} whileInView={{ x: 0 }} transition={{ duration: 0.5, ease: "easeInOut" }} className="lg:text-3xl xs:text-2xl font-bold text-textMain text-center">
                    Tools & Technologies
                </motion.h2>
                <motion.p initial={{ x: 500 }} whileInView={{ x: 0 }} transition={{ duration: 0.5, ease: "easeInOut" }} className="text-center lg:text-lg xs:text-base lg:mb-8 sm:mb-6 xs:mb-5">
                ...
                >
                    ...
                    <motion.div onClick={() => setSkillActive(1)} variants={itemVariants} className="flex flex-col items-center justify-center cursor-pointer">
                        ...
                        <p className={`font-semibold lg:text-base xs:text-sm ${skillActive == 1 ? "text-textMain" : "text-textSec"}`}>API</p>
                    </motion.div>
                ...
                <div className="flex justify-around lg:flex-row xs:flex-col items-top lg:mt-15 lg:gap-10 xs:gap-5 sm:px-10 xs:px-5 text-textMain">
                    <div className="lg:w-1/2 xs:w-full">
                        ...
                        <AnimatePresence mode="wait">
                            <motion.div key={skillActive} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.5 }}>
            ...
            <section className="lg:px-15 sm:px-8 xs:px-5 my-15">
                                ...
                                <div>
                                    ...
                                    <ul className="flex gap-2 text-sm text-gray-800 mt-3 w-3/4 cursor-default select-none">
                                        {data.utils.map((i, idx) => (
                                            <li key={idx} className="bg-bgSoft lg:py-0.5 lg:px-2 xs:px-1 lg:text-base xs:text-xs rounded-lg border border-gray-400">
                                                {i}
                                            </li>
                                        ))}
                                    </ul>
                ...
                <button onClick={() => setMoreProject(!moreProject)} className="block mx-auto mt-5 bg-mainAccent hover:bg-hoverAccent px-10 py-1.5 rounded-lg text-white font-bold">
                    {moreProject ? "See Less" : "Load More"}
                </button>
            </section>
            ...
            >
                ...
                <div className="w-1/3 h-2/3 relative xs:hidden lg:block">
                    <Image src="/codePhoto.jpg" fill alt="Code Image" className="object-cover" />
                </div>
            </motion.section>
        </div>
    );
